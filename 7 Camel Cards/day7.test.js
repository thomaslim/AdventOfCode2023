...
const puzzleInput = fs
  .readFileSync("7 Camel Cards/input.txt")
  .toString("utf-8")
  .trim();

test("Read input", () => {
  expect(functions.splitSet(demoSet).length).toEqual(5);
  expect(functions.splitSet(demoSet)[0]).toEqual({
    bid: 765,
    hand: ["3", "2", "T", "3", "K"],
  });
});
...
test("add type of hand to hands array", () => {
  expect(
    functions.parseHandsAndAddType(functions.splitSet(demoSet))[0],
  ).toEqual({ bid: 765, hand: ["3", "2", "T", "3", "K"], type: "2_1P" });
});
...
test("get score part 1", () => {
  ...
  expect(functions.getTotalWinnings(functions.splitSet(puzzleInput))).toEqual(
    249726565,
  );
});
...
test("get type of hand with Joker rule", () => {
  ...
  expect(functions.testHandWithJokerRule(["T", "2", "T", "A", "A"])).toEqual(
    "3_2P",
  );
  expect(functions.testHandWithJokerRule(["3", "2", "2", "A", "T"])).toEqual(
    "2_1P",
  );

  expect(functions.testHandWithJokerRule(["J", "T", "T", "J", "T"])).toEqual(
    "7_5OAK",
  );
  expect(functions.testHandWithJokerRule([5, 5, "J", 2, 5])).toEqual("6_4OAK");
  expect(functions.testHandWithJokerRule(["T", "2", "J", "A", "T"])).toEqual(
    "4_3OAK",
  );
  expect(functions.testHandWithJokerRule(["J", "J", "9", "J", "Q"])).toEqual(
    "6_4OAK",
  );
  expect(functions.testHandWithJokerRule(["4", "2", "T", "3", "K"])).toEqual(
    "1_HC",
  );
});
...
test("get score part 2", () => {
  expect(functions.getTotalWinnings(functions.splitSet(demoSet), true)).toEqual(
    5905,
  );
  expect(
    functions.getTotalWinnings(functions.splitSet(puzzleInput), true),
  ).toEqual(251135960);
});
