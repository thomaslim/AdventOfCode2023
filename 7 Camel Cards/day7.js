...
function getTypeOfHand(hand, withJokerRule = false) {
  ...
  if (withJokerRule && table.J && table.J !== 5) {
    ...
    delete table.J;
    const maxKey = Object.keys(table).reduce((a, b) =>
      table[a] > table[b] ? a : b,
    );
...
function parseHandsAndAddType(hands, withJokerRule = false) {
  return hands.map((hand) => ({
    ...hand,
    type: getTypeOfHand(hand.hand, withJokerRule),
  }));
}
...
function sortHands(hands, withJokerRule = false) {
  ...
  const sortedHands = handsWithType.sort((a, b) => {
    ...
    const handAvalues = a.hand.map((item) =>
      convertCardToValue(item, withJokerRule),
    );
    const handBvalues = b.hand.map((item) =>
      convertCardToValue(item, withJokerRule),
    );
    for (let i = 0; i < 5; i += 1) {
...
function getTotalWinnings(hands, withJokerRule = false) {
  ...
  const result = sortedHands.reduce(
    (acc, item, index) => acc + item.bid * (index + 1),
    0,
  );
  return result;
}
