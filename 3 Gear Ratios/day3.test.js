...
test('isNumberNextToASymbol', () => {
    expect(isNumberNextToASymbol(demoSet, '467', 0)).toEqual({"line": 1, "position": 3, "symbol": "*"})
    expect(isNumberNextToASymbol(demoSet, '617', 4)).toEqual({"line": 4, "position": 3, "symbol": "*"})
    expect(isNumberNextToASymbol(demoSet, '35', 2)).toEqual({"line": 1, "position": 3, "symbol": "*"})
    expect(isNumberNextToASymbol(demoSet, '633', 2)).toEqual({"line": 3, "position": 6, "symbol": "#"})
    expect(isNumberNextToASymbol(demoSet, '592', 6)).toEqual({"line": 5, "position": 5, "symbol": "+"})
    expect(isNumberNextToASymbol(demoSet, '755', 7)).toEqual({"line": 8, "position": 5, "symbol": "*"})
    expect(isNumberNextToASymbol(demoSet, '664', 9)).toEqual({"line": 8, "position": 3, "symbol": "$"})
    expect(isNumberNextToASymbol(demoSet, '598', 9)).toEqual({"line": 8, "position": 5, "symbol": "*"})
    expect(isNumberNextToASymbol(demoSet, '114', 0)).toBeFalsy()
    expect(isNumberNextToASymbol(demoSet, '58', 5)).toBeFalsy()
    expect(isNumberNextToASymbol(demoSet2, '12', 2, 8)).toEqual({"line": 2, "position": 7, "symbol": "-"})
})
...
function isNumberNextToASymbol(dataSet, input, lineId, position = undefined) {
    ...
    if (checkIfIsSymbol(lines[lineId][inputPosition - 1])) {
        return {symbol: lines[lineId][inputPosition -1], line:lineId, position:inputPosition -1 }
    }
    if (checkIfIsSymbol(lines[lineId][inputPosition + input.length])) {
        return {symbol: lines[lineId][inputPosition + input.length], line:lineId, position:inputPosition + input.length }
    }
    ...
    for (cursor = inputPosition-1; cursor <= inputPosition + input.length; cursor++ ) {
        ...
        if (lineId > 0) {
            if( checkIfIsSymbol(lines[lineId-1][cursor]) ) {
                return {symbol: lines[lineId-1][cursor], line:lineId-1, position:cursor }
            }
        }
...
function parseInputFile(dataSet) {
    ...
    lines.forEach((element, index) => {
        ...
        const filterNumber = (number) => {
            const symbol = isNumberNextToASymbol(dataSet, number[0], index, number.index )
            if (symbol && symbol.symbol == '*') {
                const existingSymbolIndex = symbolsAndClosedNumbers.findIndex( obj => obj.line == symbol.line && obj.position == symbol.position)
                if (existingSymbolIndex != -1) {
                    symbolsAndClosedNumbers[existingSymbolIndex].closedNumbers.push(number[0])
                } else {
    ...
    const resultPart1 = selectedNumbersFlat.reduce((acc, value) => {
        return acc + Number(value)
    }, 0)
    
    const gears = symbolsAndClosedNumbers.filter(element => element.closedNumbers.length == 2)
    const gearRatioSum = gears.reduce((acc, value) => {
        const gearRatio = value.closedNumbers.reduce((acc, value) => {
            return acc * +value
        }, 1)
