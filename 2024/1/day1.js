...
function parseInputAsString(input) {
  const result = input
    ...
    .reduce(
      (acc, line) => {
        acc.column1.push(parseInt(line[0], 10));
        acc.column2.push(parseInt(line[1], 10));
...
function computeTotalDistance(sortedObject) {
  ...
  const totalDistance = sortedObject.column1.reduce(
    (acc, item, index) =>
      acc +
      calculateDistance(
        sortedObject.column1[index],
        sortedObject.column2[index],
      ),
    0,
  );
...
function findTotalSimilarity(sortedObject) {
  return sortedObject.column1.reduce(
    (acc, item) =>
      acc + item * findSimilarityForNumber(item, sortedObject.column2),
    0,
  );
}
