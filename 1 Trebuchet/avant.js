...
function extractAndConcatenateDigits(stringList) {
    const results = [];
    ...
    for (const inputString of stringList) {
        ...
        const regexArray = [
            {regex : /one/gm, subst : 1},
            {regex : /two/gm, subst : 2},
            {regex : /three/gm, subst : 3},
            {regex : /four/gm, subst : 4},
            {regex : /five/gm, subst : 5},
            {regex : /six/gm, subst : 6},
            {regex : /seven/gm, subst : 7},
            {regex : /eight/gm, subst : 8},
            {regex : /nine/gm, subst : 9},
        ]


       // const result = inputString.replace(regexArray[1].regex, regexArray[1].subst);
        const translatedString = regexArray.reduce(function(result, item){
            return result = result.replace(item.regex, item.subst)
        }, inputString)
        console.log(inputString,' -> Substitution result: ', translatedString);

        for (const char of translatedString) {
            if (!isNaN(char) && firstDigit === null) {
                firstDigit = char;
            }
        ...
        const concatenatedDigits = firstDigit && lastDigit ? firstDigit + lastDigit : null;
...
const concatenatedResults = extractAndConcatenateDigits(shortList);
const sum = concatenatedResults.reduce(function(result, item){
    return result + Number(item.concatenatedDigits)
}, 0)

for (const result of concatenatedResults) {
    console.log(`${result.inputString} -> ${result.translatedString} -> Concatenated Digits: ${result.concatenatedDigits}`);
}
console.log(`Sum: ${sum}`)