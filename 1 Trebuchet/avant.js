...
function extractAndConcatenateDigits(stringList) {
  ...
  for (const inputString of stringList) {
    ...
    const translatedString = regexArray.reduce((result, item) => {
      const result2 = result.replace(item.regex, item.subst);
      return result2;
    }, inputString);

    for (const char of translatedString) {
      if (!Number.isNaN(char) && firstDigit === null) {
        firstDigit = char;
      }
    ...
    const concatenatedDigits =
      firstDigit && lastDigit ? firstDigit + lastDigit : null;
...
const sum = concatenatedResults.reduce(
  (result, item) => result + Number(item.concatenatedDigits),
  0,
);

for (const result of concatenatedResults) {
  console.log(
    `${result.inputString} -> ${result.translatedString} -> Concatenated Digits: ${result.concatenatedDigits}`,
  );
}
console.log(`Sum: ${sum}`);
